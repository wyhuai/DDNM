...
The High-Quality results presented in the front figure are mostly generated by applying DDNM to [RePaint](https://github.com/andreas128/RePaint), which uses time-travel trick based on DDPM sampling (DDPM is not yet implemented in this repository). By the way, we find that using DDIM (without time-travel) usually yields better results than DDPM (without time-travel). However, DDPM (with time-travel) yields better results than DDIM (with time-travel).
