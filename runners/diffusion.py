...
from functions.denoising import ddnm_diffusion, ddnm_plus_diffusion
...
class Diffusion(object):
    ...
    def simplified_ddnm_plus(self, model, cls_fn):
        ...
        if args.deg =='colorization':
            ...
            A = lambda z: z*mask
            Ap = A
        elif args.deg =='old photo restoration':
            loaded = np.load("exp/inp_masks/mask.npy")
            mask = torch.from_numpy(loaded).to(self.device)
            A1 = lambda z: z*mask
            A1p = A1
...
#             for i in range(len(Apy)): # visualize y, Apy, and x_orig
#                 tvu.save_image(
#                     inverse_data_transform(config, y[i].view(1, 3, 64, 64)),
#                     os.path.join(self.args.image_folder, f"y_{idx_so_far + i}.png")
#                 )
#                 tvu.save_image(
#                     inverse_data_transform(config, Apy[i]),
#                     os.path.join(self.args.image_folder, f"Apy_{idx_so_far + i}.png")
#                 )
#                 tvu.save_image(
...
#                     if i%50==0: # visualize the intermediate results
...
#                         tvu.save_image(
#                             inverse_data_transform(config, x0_t_hat[0]),
...
#                         tvu.save_image(
#                             inverse_data_transform(config, xt_next[0]),
...
#                 )
            ...
            with torch.no_grad():
                if sigma_y==0.:
                    #noise-free case, turn to ddnm
                    x, _ = self.ddnm(x, model, A_funcs, y, last=False, cls_fn=cls_fn, classes=classes)
                else:
                    #noisy case, turn to ddnm+
                    x, _ = self.ddnm_plus(x, model, A_funcs, y, sigma_y, last=False, cls_fn=cls_fn, classes=classes)
    ...
    def ddnm(self, x, model, A_funcs, y, last=True, cls_fn=None, classes=None):
        skip = self.num_timesteps // self.args.timesteps
        seq = range(0, self.num_timesteps, skip)

        x = ddnm_diffusion(x, seq, model, self.betas, A_funcs, y, cls_fn=cls_fn, classes=classes)

        if last:
            x = x[0][-1]
        return x
    
    def ddnm_plus(self, x, model, A_funcs, y, sigma_y, last=True, cls_fn=None, classes=None):
        skip = self.num_timesteps // self.args.timesteps
        seq = range(0, self.num_timesteps, skip)

        x = ddnm_plus_diffusion(x, seq, model, self.betas, A_funcs, y, sigma_y, cls_fn=cls_fn, classes=classes)
